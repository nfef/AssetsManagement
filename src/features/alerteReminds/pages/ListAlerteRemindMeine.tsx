...
const ListAlerteRemindMeine = () => {

  const navigate = useNavigate();
  const currentUser = useSelector(selectCurrentUser);
  const [queryFilter,setQueryFilter] = useState("creator@="+currentUser?.preferred_username);
    ...
    const target = e.target as HTMLInputElement;
      ...
      Timer = setTimeout(function () {
          if (target.value.length >= 3) {
            setQueryFilter(
              "(creator|remindInterval|remindNumber|remindDate|contractType|motif|finalizationDate|finalizer|actions|finalizer)@=" + target.value + ",creator@="+currentUser?.preferred_username
            );
          }else if(target.value.length == 0) {
            setQueryFilter("creator@="+currentUser?.preferred_username);
          }
      }, 2000);
  };
  ...
  return (
      <div className="p-4">
        <div className="mb-2">
          <div className="mb-2">Les alertes de rappels que vous avez crée.</div> 
